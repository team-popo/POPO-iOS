...
public class TrackerAPI {
    ...
    func getOptions(popoId: Int, completion: @escaping (NetworkResult<Any>) -> Void) {
        
        trackerProvider.request(.getOptions(popoId: popoId)) { (result) in
            switch result {
            case .success(let response):
                let statusCode = response.statusCode
                let data = response.data
                
                let networkResult = self.judgeGetOptionsStatus(by: statusCode, data)
                completion(networkResult)
    ...
    private func judgeGetOptionsStatus(by statusCode: Int, _ data: Data) -> NetworkResult<Any> {
        
        let decoder = JSONDecoder()
        guard let decodedData = try? decoder.decode(GenericResponse<[TrackerOptions]>.self, from: data)
        else {
            return .pathErr
        }
